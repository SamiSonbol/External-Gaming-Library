...
void Executable::set_path(const std::filesystem::path& path) {
...
void Executable::launch() {
	...
	if (!this->path.empty()) {
		...
		LPCTSTR exe_path = this->path.string().c_str();
...
Executable::Executable(const std::filesystem::path &path) {
	...
	std::cerr << "WARNING: couldnt find executable path => dead Executable object created\n";
